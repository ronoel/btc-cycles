...
---
**Last research snapshot: Aug 12, 2026** · next manual refresh due: **late Aug 2026** (switch to **weekly** from September — projected bottom window)

Carry into the next pass: whether the **exchange-reserve uptick and LTH pullback wash out** once the Coldcard migration settles (both rows are PARTIAL pending exactly that); the **30-year auction result (Aug 13)** and the **BLS payrolls benchmark revision (Aug 28**, flagged in advance as large); the **difficulty retarget ~Aug 22** (a positive print after +0.99% starts the case for a genuine Hash Ribbons cross); the **Section 338 Canada tariffs landing Aug 19** and the pending Federal Circuit Section 122 ruling; Strategy's weekly 8-Ks (watch basic mNAV ~0.68× and whether sale tranches accelerate past the ~1,600–1,700 BTC/week pattern); and the **Sep 15–16 FOMC with a dot plot** — hold is priced (~62%) after the hike scare deflated, so the shock scenario is now a hike nobody is positioned for. Inside the window: **Mt. Gox Oct 31** (trustee moved 10,422 BTC in June; no distribution round confirmed) and **US midterms Nov 3**.

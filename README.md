...
> The headline finding of the current build: the seven scoring rules with clean history in both eras read **100% at the confirmed Nov 21, 2022 bottom** and **34% today**. That is what makes the readiness number mean something rather than assert something.
...
- **On-chain metrics** (Realized Price, MVRV ratio **and** MVRV Z-Score, NUPL, SOPR, Puell): updated daily at 06:00 UTC by a GitHub Action running [`scripts/build_data.py`](scripts/build_data.py) → `data.json` (latest) + `history.json` (~4y of daily series, used only by the retro-calibration). Source: bitcoin-data.com / BGeometrics
